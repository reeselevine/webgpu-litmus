...
[[stage(compute), workgroup_size(workgroupXSize)]] fn main(
  ...
  if (shuffled_workgroup < stress_params.testing_workgroups) {
    ...
    workgroupBarrier();
    let unused = atomicExchange(&wg_test_locations[y_0], 1u);
    let unused1 = atomicExchange(&wg_test_locations[y_1], 2u);
    workgroupBarrier();
    atomicStore(&wg_test_locations[x_1], 1u);
    workgroupBarrier();
    atomicStore(&test_locations.value[shuffled_workgroup * u32(workgroupXSize) * stress_params.mem_stride * 2u + x_1], atomicLoad(&wg_test_locations[x_1]));
