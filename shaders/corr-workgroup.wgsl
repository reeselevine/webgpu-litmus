...
var<workgroup> workgroup_test_locations: array<atomic<u32>, 1024>;
...
[[stage(compute), workgroup_size(workgroupXSize)]] fn main(
  ...
  if (shuffled_workgroup < stress_params.testing_workgroups) {
    let x_first = local_invocation_id[0];
    let x_second = permute_id(local_invocation_id[0], stress_params.permute_first, u32(workgroupXSize));
    let y_second = permute_id(x_second, stress_params.permute_second, u32(workgroupXSize));
    ...
    atomicStore(&workgroup_test_locations[x_first*stress_params.mem_stride*2u], 1u);
    let r1 = atomicLoad(&workgroup_test_locations[y_second*stress_params.mem_stride*2u + stress_params.location_offset]);
    let r0 = atomicLoad(&workgroup_test_locations[x_second*stress_params.mem_stride*2u]);
    storageBarrier();
    if (r0 == 0u && r1 == 0u) {
