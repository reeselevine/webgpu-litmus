...
@compute @workgroup_size(workgroupXSize) fn main(
  ...
  if (shuffled_workgroup < stress_params.testing_workgroups) {
    let total_ids = (workgroupXSize * stress_params.testing_workgroups)/2;
    let id_0 = shuffled_workgroup * workgroupXSize + local_invocation_id[0];
    let id_0_final = id_0 % total_ids;
    let id_0_first_half = id_0 / total_ids == 0;
    ...
    let id_1_final = id_1 % total_ids;
    let id_1_first_half = id_1 / total_ids == 0;
    var mem_0: u32;
    if (id_0_first_half) {
      mem_0 = id_0_final * stress_params.mem_stride * 2u;
    } else {
      mem_0 = permute_id(id_0_final, stress_params.permute_second, total_ids) * stress_params.mem_stride * 2u + stress_params.location_offset;
    }
    let x_1 = id_1_final * stress_params.mem_stride * 2u;
    let y_1 = permute_id(id_1_final, stress_params.permute_second, total_ids) * stress_params.mem_stride * 2u + stress_params.location_offset;
    ...
    atomicStore(&test_locations.value[mem_0], 1u);
    
    if (id_1_first_half) {
      let r0 = atomicLoad(&test_locations.value[x_1]);
      let r1 = atomicLoad(&test_locations.value[y_1]);
      atomicStore(&results.value[id_1_final].r1, r1);
      atomicStore(&results.value[id_1_final].r0, r0);
    } else {
      let r2 = atomicLoad(&test_locations.value[y_1]);
      let r3 = atomicLoad(&test_locations.value[x_1]);
      atomicStore(&results.value[id_1_final].r3, r3);
      atomicStore(&results.value[id_1_final].r2, r2);
    }
