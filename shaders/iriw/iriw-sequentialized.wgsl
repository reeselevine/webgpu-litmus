...
@compute @workgroup_size(workgroupXSize) fn main(
  ...
  if (shuffled_workgroup < stress_params.testing_workgroups) {
    ...
    let x_1 = id_0_final * stress_params.mem_stride * 2u;
    let y_1 = permute_id(id_0_final, stress_params.permute_second, total_ids) * stress_params.mem_stride * 2u + stress_params.location_offset;
    ...
    if (id_0_first_half) {
      ...
      atomicStore(&results.value[id_0_final].r1, r1);
      atomicStore(&results.value[id_0_final].r0, r0);
    } else {
      let r2 = atomicLoad(&test_locations.value[y_1]);
      let r3 = atomicLoad(&test_locations.value[x_1]);
      atomicStore(&results.value[id_0_final].r3, r3);
      atomicStore(&results.value[id_0_final].r2, r2);
    }
