...
[[stage(compute), workgroup_size(workgroupXSize)]] fn main(
  ...
  if (shuffled_workgroup < stress_params.testing_workgroups) {
    ...
    atomicStore(&test_locations.value[x_first*stress_params.mem_stride*2u], 2u);
    atomicStore(&test_locations.value[y_first*stress_params.mem_stride*2u + stress_params.location_offset], 1u);
    let r0 = atomicLoad(&test_locations.value[y_second*stress_params.mem_stride*2u + stress_params.location_offset]);
    atomicStore(&test_locations.value[x_second*stress_params.mem_stride*2u], 1u);
    storageBarrier();
    let mem_result = atomicLoad(&test_locations.value[x_second*stress_params.mem_stride*2u]);
    if (r0 == 1u && mem_result == 1u) {
      atomicAdd(&results.seq0, 1u);
    } elseif (r0 == 0u && mem_result == 2u) {
      atomicAdd(&results.seq1, 1u);
    } elseif (r0 == 0u && mem_result == 1u) {
      atomicAdd(&results.interleaved, 1u);
    } elseif (r0 == 1u && mem_result == 2u) {
