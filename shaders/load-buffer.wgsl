...
[[stage(compute), workgroup_size(workgroupXSize)]] fn main(
  ...
  if (shuffled_workgroup < stress_params.testing_workgroups) {
    ...
    let y_0 = &test_locations.value[id_0 * stress_params.mem_stride * 2u];
    let x_0 = &test_locations.value[permute_id(id_0, stress_params.permute_second, total_ids) * stress_params.mem_stride * 2u + stress_params.location_offset];
    let x_1 = &test_locations.value[permute_id(id_1, stress_params.permute_second, total_ids) * stress_params.mem_stride * 2u + stress_params.location_offset];
    let y_1 = &test_locations.value[id_1 * stress_params.mem_stride * 2u];
